...
def get_network(config):
    ...
    if config['architecture'] == 'T2T-ViT-14':
        ...
        network = T2t_vit_14(num_classes=config['n_classes'])
        if config['pretrained']:
            url_pretrained = MODELS_URLS['T2T-ViT-14']
            state_dict = torch.load(url_pretrained, map_location='cpu')['state_dict_ema']
            del state_dict['head' + '.weight']
            del state_dict['head' + '.bias']
            network.load_state_dict(state_dict, strict=False)
    if config['architecture'] == 'T2T-ViT_t-24':
        from .T2T.models import T2t_vit_t_24
        network = T2t_vit_t_24(num_classes=config['n_classes'])
        if config['pretrained']:
    ...
    if config['architecture'] == 'T2T-ViT-19':
        ...
        network = T2t_vit_19(num_classes=config['n_classes'])
        if config['pretrained']:
